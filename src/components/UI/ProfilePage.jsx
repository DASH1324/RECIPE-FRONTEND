...
function ProfilePage() {
  // --- A single state for all form fields ---
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const response = await axios.get('https://recipe-ctre.onrender.com/auth/users/me', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        'https://recipe-ctre.onrender.com/auth/users/me',
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const newProfileState = {
        username: username,
        email: formData.email, // Email doesn't change on its own, so keep the form value
        fullName: full_name,
        currentPassword: '',
        newPassword: '',
        confirmNewPassword: '',
      ...
      setInitialData(newProfileState); // Update the base state
