...
const LoginForm = ({ onForgotPassword = () => {} }) => {
  ...
  // MODIFIED: This function is updated to send form data
  const handleSubmit = async (values) => {
    setIsLoading(true);

    // 1. Create a URLSearchParams object to format the data
    const formData = new URLSearchParams();
    formData.append('username', values.username);
    formData.append('password', values.password);

    try {
      // 2. Send the formData object. Axios will automatically set the
      //    'Content-Type' header to 'application/x-www-form-urlencoded'.
      const response = await axios.post("https://recipe-ctre.onrender.com/auth/token", formData);
      
      // Note: The key in your code is "access_token". Make sure this is consistent
      // with what you use in other components (e.g., ProfilePage.jsx)
